...
from rgb2normal_pipeline import MonoNormPipeline
import glob
import pdb
...
def xatlas_uvmap1(glctx, geometry, mat, FLAGS):
    ...
    mask, normal = render.render_uv1(glctx, new_mesh, FLAGS.texture_res, eval_mesh.material['bump'], FLAGS.uv_padding_block)
...
def initial_material(FLAGS, geometry):
    ...
    mlp_min = nrm_min #torch.cat((nrm_min), dim=0)
    mlp_max = nrm_max #torch.cat((nrm_max), dim=0)
...
def optimize_mesh(
    ...
    for it, target in enumerate(dataloader_train):
        ...
        total_loss = img_loss + reg_loss + mse_loss
...
if __name__ == "__main__":
    ...
    parser.add_argument("--guidance_type", type=str, default="rgb2normal_mse")
    parser.add_argument("--rgb2normal_pretrained_model", type=str, default="")
    ...
    if FLAGS.guidance_type == 'rgb2normal_mse':
        guidance = pipe = MonoNormPipeline.from_pretrained(
            FLAGS.rgb2normal_pretrained_model,
            torch_dtype=torch.float32,            
        ).to('cuda')
        guidance.unet.requires_grad_(False)
        guidance.vae.requires_grad_(False)
        guidance.text_encoder.requires_grad_(False)
    else:
