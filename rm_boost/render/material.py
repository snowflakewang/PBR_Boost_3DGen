    ...
    for mat in materials:
        ...
        if 'bump' in mat:
            ...
            def process_bump(x):
                x[...,0:2] = x[...,0:2] * 2.0 - 1.0
                return x
            mat['normal'] = texture.load_texture2D(os.path.join(mtl_path, mat['bump']), lambda_fn=lambda x: process_bump(x), channels=3)
            '''
            #mat['normal'] = texture.load_texture2D(os.path.join(mtl_path, mat['bump']), channels=3)
        ...
        if material is not None:
            ...
            if 'normal' in material.keys():
                ...
                def process_bump(x):
                    x[...,0:2] = (x[...,0:2] + 1) * 0.5
                    return x
                texture.save_texture2D(os.path.join(folder, 'texture_n.png'), material['normal'], lambda_fn=lambda x: process_bump(x))
                '''
