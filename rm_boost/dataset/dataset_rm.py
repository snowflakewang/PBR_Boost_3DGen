...
class DatasetMesh(torch.utils.data.Dataset):
    ...
    def __init__(self, glctx, FLAGS, validate=False, gif=False):
        ...
        self.ks_reference = dict() # input/warbear_al_crm/ks_mask/ks
        img_files = sorted(os.listdir(self.FLAGS.ks_path))
        for f in img_files:
            if f.split('.')[-1] in ['jpg', 'png']:
                ks = np.array(imageio.imread(f'{self.FLAGS.ks_path}/{f}'), dtype=np.float32)
                ks = torch.tensor(ks, requires_grad=False) / 255.0 # [0,255]->[0,1]
                if f.split('_')[1] == '%06d'%0:
                    self.ks_reference[0] = ks
                elif f.split('_')[1] == '%06d'%25:
                    self.ks_reference[1] = ks
                elif f.split('_')[1] == '%06d'%50:
                    self.ks_reference[2] = ks
                elif f.split('_')[1] == '%06d'%75:
                    self.ks_reference[3] = ks
                elif f.split('_')[1] == '%06d'%100:
                    self.ks_reference[4] = ks
                elif f.split('_')[1] == '%06d'%101:
                    self.ks_reference[5] = ks
    ...
    def _gif_scene(self, itr):
        ...
        if itr < 100:
            ang    = (itr / 100) * np.pi * 2
            rotate_x =  np.deg2rad(20) # +-np.pi/2
            #rotate_x = 0
        elif itr == 100:
            ang = 0
            rotate_x = np.pi/2
        ...
        return mv[None, ...], mvp[None, ...], campos[None, ...], self.FLAGS.display_res, self.FLAGS.spp, normal_rotate[None,...], prompt_index, None # ks_refer is None here
    ...
    def _validate_scene(self, itr):
        ...
        mvp    = proj_mtx @ mv
        campos = torch.linalg.inv(mv)[:3, 3]
        return mv[None, ...], mvp[None, ...], campos[None, ...], self.FLAGS.display_res, self.FLAGS.spp, normal_rotate[None,...], prompt_index, None
    ...
    def _mv_fusion_scene(self, itr):
        angs = [0, 0.25*np.pi*2, 0.5*np.pi*2, 0.75*np.pi*2, 0, 0]
        rotate_xs = [np.deg2rad(20)] * 4 + [np.pi/2, -np.pi/2]
        if self.FLAGS.gpu_number == 1:
            fovy = np.deg2rad(45)
            proj_mtx = util.perspective(fovy, self.FLAGS.display_res[1] / self.FLAGS.display_res[0], self.FLAGS.cam_near_far[0], self.FLAGS.cam_near_far[1])

            #ang    = (itr / 100) * np.pi * 2
            #rotate_x =  np.deg2rad(20) # +-np.pi/2
            ang_idx = itr % len(angs) # [0,5]
            #ang_idx = np.random.randint(low=0, high=len(ang))
            ang = angs[ang_idx]
            rotate_x = rotate_xs[ang_idx]
            ...
            ks_reference = self.ks_reference[ang_idx].unsqueeze(0)

            return mv[None, ...], mvp[None, ...], campos[None, ...], self.FLAGS.display_res, self.FLAGS.spp, normal_rotate[None,...], prompt_index, ks_reference
    ...
    def __getitem__(self, itr):
        if self.gif:
            mv, mvp, campos, iter_res, iter_spp, normal_rotate, prompt_index, ks_reference = self._gif_scene(itr)
        elif self.validate:
            mv, mvp, campos, iter_res, iter_spp, normal_rotate, prompt_index, ks_reference = self._validate_scene(itr)
        else:
            #mv, mvp, campos, iter_res, iter_spp, normal_rotate, prompt_index = self._train_scene(itr)
            mv, mvp, campos, iter_res, iter_spp, normal_rotate, prompt_index, ks_reference = self._mv_fusion_scene(itr)
    ...
    def collate(self, batch):
        ...
        return {
            ...
            'ks_refer': torch.cat(list([item['ks_refer'] for item in batch]), dim=0) if batch[0]['ks_refer'] is not None else None
        }
