...
class CRM(nn.Module):
    ...
    def export_mesh_wt_uv(self, ctx, data, out_dir, ind, device, res, tri_fea_2=None):
        ...
        gb_pos, _ = interpolate(mesh_v[None, ...], rast, mesh_pos_idx.int()) # gb_pos.shape=[B,1024,1024,3]=[B,H,W,3]
        mask = rast[..., 3:4] > 0 # mask.shape=[B,1024,1024,3]
        ...
        gb_mask_pos = gb_pos_unsqz[mask_unsqz] # gb_mask_pos.shape=[num_mask_equal_1,3], num_mask_equal_1=mask.sum()

        gb_mask_pos = gb_mask_pos[None, ]

        with torch.no_grad():

            dec_verts = self.decoder(tri_fea_2, gb_mask_pos) # dec_verts.shape=[B,num_mask_equal_1,32]
            colors = self.rgbMlp(dec_verts).squeeze() # colors.shape=[num_mask_equal_1,3]
        ...
        tex = tex_unsqz.view(res + (3,)) # tex.shape=[1024,1024,3], tex.range=[0,255], tex here is the UV map
        ...
        fid.write(f'map_Ks {out_dir.split("/")[-1]}_ks.png\n')
        fid.close()
        ...
        '''
        ks_mask = (img[...,0:1] > 10) * (img[...,1:2] > 10) * (img[...,2:3] > 10) * 1.0
        
        metal = 0.9 * ks_mask + 0.1 * (1 - ks_mask)
        rough = 0.1 * ks_mask + 0.9 * (1 - ks_mask)
        
        metal = 0.3 * ks_mask + 0.7 * (1 - ks_mask)
        rough = 0.6 * ks_mask + 0.4 * (1 - ks_mask)

        ks = np.concatenate([np.zeros_like(metal), rough, metal], axis=-1) * 255.0 # [o,r,m]
        dilate_ks = cv2.dilate(ks, kernel, iterations=1)
        ks = ks * (1 - mask) + dilate_ks * mask
        ks = ks.clip(0, 255).astype(np.uint8)
        cv2.imwrite(f'{out_dir}_ks.png', ks[..., [2, 1, 0]])
        '''