...
def shade(
    ...
    # Texture lookups
    ################################################################################
    if 'bump' in material and mode == 'appearance_modeling':
        bump = material['bump'].sample(gb_texc, gb_texc_deriv)[...,0:3]
        albedo = material['albedo'].sample(gb_texc, gb_texc_deriv)[...,0:3]
        rm = material['rm'].sample(gb_texc, gb_texc_deriv)[...,0:3]
        assert bump.shape[-1] == 3 and albedo.shape[-1] == 3 and rm.shape[-1] == 3
    else:
        raise NotImplementedError
    alpha = albedo[..., 3:4] if albedo.shape[-1] == 4 else torch.ones_like(albedo[..., 0:1])  #[1,512,512,1]
    albedo = albedo[..., 0:3]
    ################################################################################
    ...
    gb_normal1 = gb_normal #TODO why Fantasia3D authors need the following random rotate?
    ...
    if bsdf == 'pbr':
        if mode == 'geometry_modeling':
            shaded_col = albedo * ru.lambert(gb_normal, util.safe_normalize(view_pos - gb_pos)) * 5
        elif mode == 'appearance_modeling':
            shaded_col = lgt.shade(gb_pos, gb_normal, albedo, rm, view_pos, specular = True)
        else:
        ...
        if mode == 'geometry_modeling':
            ...
            shaded_col = lgt.shade(gb_pos, gb_normal, albedo, rm, view_pos, specular = False)
        else:
            assert False, "Invalid mode type"
    elif bsdf == 'normal':
    ...
    buffers = {
        ...
        'normal': torch.cat((gb_normal1, alpha), dim=-1),
        'albedo': torch.cat((albedo, alpha), dim=-1),
        'rm': torch.cat((rm, alpha), dim=-1),
    }
    return buffers
...
def render_uv1(ctx, mesh, resolution, mlp_texture, uv_padding_block):
    ...
    # Sample out textures from MLP
    all_tex = mlp_texture.sample(gb_pos)
    assert all_tex.shape[-1] == 3, "bump map is a 3-channel map"
    print(f'[INFO] Map bump from [[-1,1],[-1,1],[0,1]] to [[0,1],[0,1],[0.5,1]] in order to be the same as Kd and Ks...')
    all_tex = (all_tex + 1.0) * 0.5
    print(f'[INFO] UV padding for bump...')
    perturbed_nrm = uv_padding(all_tex, hole_mask, uv_padding_block)
    
    return (rast[..., -1:] > 0).float(), perturbed_nrm 